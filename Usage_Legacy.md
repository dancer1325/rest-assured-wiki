...
**Note that the usage guide for newer versions of REST Assured is located at the [Usage](Usage) page.**
...
# Filters #
A filter allows you to inspect and alter a request before it's actually committed and also inspect and [alter](#Response_Builder) the response before it's returned to the expectations. You can regard it as an "around advice" in AOP terms. Filters can be used to implement custom authentication schemes, session management, logging etc. To create a filter you need to implement the [com.jayway.restassured.filter.log.Filter](http://rest-assured.googlecode.com/svn/tags/1.9.0/apidocs/com/jayway/restassured/filter/Filter.html) interface. To use a filter you can do:
